...
static ssize_t demo_atomic_read(struct file *filp, char __user *buf, size_t count,
			 loff_t *off)
{
	char tmp[64];
	loff_t pos;
...
static ssize_t demo_atomic_write(struct file *filp, const char __user *buf, size_t len,
			  loff_t *off)
{
	long val;
	int ret;
...
static int __init demo_atomic_driver_init(void)
{
	struct device *cdev;
	int retval;
	dev_t dev;
	
	retval = alloc_chrdev_region(&dev, 0, 1, "demo_atomic_dev");
	if (retval < 0) {
	...
	demo_atomic_class = class_create(THIS_MODULE, "atomic_demo");
...
static void __exit demo_atomic_driver_exit(void)
{
...
module_init(demo_atomic_driver_init);
module_exit(demo_atomic_driver_exit);
